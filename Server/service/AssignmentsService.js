                    ...
                    if (err) {
                      ...
                      db.run(sql5, [userId, taskId, userId, taskId], function (err) {
                        if (err) {
                          ...
                          var message = new MQTTMessage("UPDATE", 'active', parseInt(userId), rows2[0].name, taskId, null)
                          ...
                          //publish the MQTT message for the selected task
                          if (deselected) {
                            var message = new MQTTMessage("UPDATE", "inactive", null, null, parseInt(taskId), null);
                            //var message = new MQTTMessage("inactive", null, null);
                            mqtt.saveMessage(deselected, message);
                            mqtt.publishTaskMessage(String(deselected), message);
                          }
